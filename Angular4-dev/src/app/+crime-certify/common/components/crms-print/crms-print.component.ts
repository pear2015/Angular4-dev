...
@Component({
    selector: 'crms-print',
    templateUrl: './crms-print.component.html',
    // styleUrls: ['./essence-ng2-print.component.scss']
})
export class CrmsPrintComponent implements OnInit {
    @Input() mode: string;
    ...
    private getBody() {
        ...
        return `<body>${html}</body>` + `<style type="text/css">ol{list-style:none} .sr-only{display: none !important;}</style>`;
    }
    ...
    private createIframe() {
        ...
        try {
            ...
            this.printDoc = printIframe.contentDocument ? printIframe.contentDocument : (printIframe.contentWindow ?
                printIframe.contentWindow.document : printIframe.document);
        } catch (e) {
